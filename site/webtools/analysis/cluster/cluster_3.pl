...
my $cmd =<<EOF;
analysis/Rwrapper.cgi \\
 --r_exe=R \\
 --kind $kind \\
 --infile=$infile \\
 --outfile=$outfile \\
 --settings logBase2=$settings{logBase2} \\
 --settings euclidDist=$settings{euclidDist} \\
 --settings outgraph=$settings{outgraph} \\
 --settings outuser=$settings{outuser} \\
 --settings outstat=$settings{outstat} \\
 --settings agglomeration=$settings{agglomeration} \\
 --settings path=$settings{path} \\
 --settings zoom=$zoom
EOF
print "Command is $cmd\n";
system($cmd) and die "Rwrapper returned error";
