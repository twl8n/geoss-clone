...
# first run permCluster code

my $cmd = "analysis/Rwrapper.cgi --kind $kind --infile=$infile --outfile=$outfile --settings logBase2=$settings{logBase2} --settings euclidDist=$settings{euclidDist} --logfile=$logfile --settings outgraph=$settings{outgraph} --settings outuser=$settings{outuser} --settings outstat=$settings{outstat} --settings agglomeration=$settings{agglomeration}  --settings path=$settings{path} --settings zoom=$zoom";
warn "Command is $cmd";
`$cmd`;
warn "Done Command";
