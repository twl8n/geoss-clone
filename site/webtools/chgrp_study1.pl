...
main:
{
    ...
    (my $all_html, my $loop_template) = readtemplate("chgrp_study1.html", "/site/webtools/header.html", "/site/webtools/footer.html");
    ...
    # 
    my $sth = getq("studies_i_own", $dbh, $us_fk);
    ...
    $ch{htmltitle} = "Manage Group Ownership and Permissions for Studies";
    $ch{help} = set_help_url($dbh,
      "change_group_ownership_and_read_or_write_permissions_for_existing_array_studies");
    $ch{htmldescription} = "Change the group ownership and/or group read-write permissions.  The experimental conditions for each array study will be changed to reflect study permissions as appropriate."; 
